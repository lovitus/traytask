...
- 环境变量管理
  ...
  - 任务级环境变量（覆盖/补充全局变量）
- 日志限制
  - 每个任务仅保留最近 `1000` 行日志（内存环形缓冲）
  - 不写日志文件到磁盘，避免频繁写盘
  - 管理台支持“清空日志”按钮（按任务清空）
- 安全执行（降低误报/拦截概率）
  - 默认优先直接执行程序（避免无必要 `cmd.exe /C`）
  - 仅在检测到管道/重定向等 shell 语法时自动走 shell
  - Windows 下任务进程绑定生命周期控制，主程序退出时自动清理子进程，避免僵尸进程累积
  - Windows 下同一任务使用命名互斥锁，跨进程也只允许一个运行实例
  - 长期无 Cron 任务额外按命令内容加全局互斥锁，避免重复实例导致同命令被并行拉起
...
配置默认存储在系统配置目录的 `traytask` 子目录：

- 配置：`config.json`
- 日志：仅内存保留最近 1000 行（程序退出后清空）
